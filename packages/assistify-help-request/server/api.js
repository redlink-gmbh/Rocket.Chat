	...
	_createHelpDiscussion(support_area, seeker, providers, message, environment = {}) {
		...
		try {
			Meteor.runAsUser(seekerUser._id, () => {
				channel = Meteor.call('createRequestFromExpertise', `Assistify_${ HelpRequestApi.getNextAssistifyRoomCode() }`, support_area, '', providerUsers.map((user) => user.username), environment);
				try {
					if (message) {
